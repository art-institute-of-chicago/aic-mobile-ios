...
class SeeAllTourCell: UICollectionViewCell {
	...
	var tourModel: AICTourModel? = nil {
		didSet {
			...
			shortDescriptionTextView.attributedText = attributedStringWithLineHeight(text: self.tourModel!.shortDescription, font: .aicTextFont, lineHeight: 20)
			stopsNumberLabel.text = "tour_stop_count"
				.localizedFormat(arguments: "\(tourModel.stops.count)", using: "Base")
