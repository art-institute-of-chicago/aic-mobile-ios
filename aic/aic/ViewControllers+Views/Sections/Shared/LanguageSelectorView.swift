...
class LanguageSelectorView: UIView {
	...
	var languageAButtonTopMargin: NSLayoutConstraint?
	var languageBButtonTopMargin: NSLayoutConstraint?
	var selectedLanguageButtonTopMargin: NSLayoutConstraint?
	...
	var language: Common.Language = .english {
		didSet {
			...
			availableLanguages.sort { (first, second) -> Bool in
				if first == language { return true } else if second == language { return false } else if first == .english { return true } else if second == .english { return false } else if first == .spanish { return true } else if second == .spanish { return false } else if first == .chinese { return true } else if second == .chinese { return false }
				return false
			}

			// reset button languages in order starting from the selected at the bottom
			for index in 0...availableLanguages.count-1 {
				if index == 0 { selectedLanguageButton.language = availableLanguages[index] } else if index == 1 { languageBButton.language = availableLanguages[index] } else if index == 2 { languageAButton.language = availableLanguages[index] }
			}
		}
	}
